...
func die(reason: String = "") -> void:
	...
	# 1. СИСТЕМА УГАРНЫХ ПРИНТОВ СМЕРТИ
	match reason:
		"makarov_suicide":
			print("\nты блять долбаёб нахуй? не блять... аэээ")
		"suicide":
			print("...")
		_:
			print("\nты умер.")
			# Плавный вход паники/размытия через код (так как зависит от материала)
			var shader_mat = panic_overlay.material as ShaderMaterial
			if shader_mat:
				create_tween().tween_property(shader_mat, "shader_parameter/panic_intensity", 1.0, 0.2)

	# 2. ФИЗИКА И ВВОД
	Input.mouse_mode = Input.MOUSE_MODE_VISIBLE
	_change_collision_height(true) 
	if reason == "suicide":
		anim_suicide.play("boom")
	else:
		anim_suicide.play("noo")
	if blood_rect:
		var blood_tween = create_tween()
		blood_tween.tween_property(blood_rect, "color:a", 0.95, 0.8)
	# 4. РАБОТА С МУЗЫКОЙ
	_fade_out_and_switch_music()

# Вынесли музыку в отдельный метод, чтобы код смерти читался легко
func _fade_out_and_switch_music() -> void:
	if bg_music and bg_music.playing:
		var music_tween = create_tween()
		music_tween.tween_property(bg_music, "volume_db", -18.0, 0.5)\
	...
	if dead_music and not dead_music.playing:
		dead_music.play()
