...
@onready var anim_player: AnimationPlayer = $suicd 
...
func _unhandled_input(event: InputEvent) -> void:
	...
	# НАЖАТИЕ КОЛЁСИКА — НАЧАЛО ПЕРЕЗАРЯДКИЁ
...
func _switch_weapon(new_weapon: Weapon) -> void:
	...
	print("--- СРАБОТАЛА СМЕНА ОРУЖИЯ ---")
	print("Значение anim_player: ", anim_player)
	
	if anim_player != null:
		if new_weapon == Weapon.MAKAROV:
			anim_player.play("get_PM")
		elif new_weapon == Weapon.PP:
			# Если для ПП пока нет анимации, можно временно оставить пустой или закомментировать
			pass 
